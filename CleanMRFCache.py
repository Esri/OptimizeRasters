...
class Cleaner:
    ...
    def init(self, input_path, extensions = ()):
        ...
        if (self.m_input_path.endswith('/') == False):
            self.m_input_path += '/'
        self.m_info = []
        return True
    ...
    def getFreeDiskSpace(self, input_path):      # static
        try:
            ...
            ctypes.windll.kernel32.GetDiskFreeSpaceExW(ctypes.c_wchar_p(input_path), None, None, ctypes.pointer(fbytes))
        except:
            return -1
        return fbytes
    ...
    def getFileInfo(self, root_only = False):
        ...
        for r,d,f in os.walk(self.m_input_path):
            ...
            for file in f:
                ...
                if ((e_[1:].lower() in self.m_extensions) == True):
...
parser = argparse.ArgumentParser(description='Cleans MRF Cache files by oldest access-time until free space \
requested has been achieved.\n')

parser.add_argument('-input', help='Input directory', dest='input_path');
parser.add_argument('-mode', help='Processing mode. Valid modes [del]', dest='mode', default='scan');
parser.add_argument('-ext', help='Extensions to filter-in. e.g. -ext=mrfcache,txt', dest='ext');
parser.add_argument('-size', type=long, help='Free size requested in bytes. e.g. -size=1000000', dest='size', default=2000000000);
...
if (space_available == -1): # an error has occured
    Message ('Err: Unable to get the free-disk-space for the path (%s)' % (args.input_path))
    exit(1)
# ends
